...
def test_build_params_empty_dict_overrides_treated_as_falsy():
    # Empty dict is falsy so the if-branch is skipped; no update occurs
    result = build_params("mykey", {})
    assert result == {"access_key": "mykey"}


def test_build_params_access_key_preserved_alongside_overrides():
...
def test_build_params_multiple_override_keys():
    overrides = {"search": "London", "limit": 100}
    result = build_params("k", overrides)
    assert result["search"] == "London"
