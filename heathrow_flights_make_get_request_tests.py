...
def test_make_get_request_returns_json_on_success():
    ...
    with patch("heathrow_flights.requests.get", return_value=mock_response):
...
def test_make_get_request_calls_get_with_correct_url_and_params():
    ...
    params = {"access_key": "abc", "dep_iata": "LHR"}

    with patch("heathrow_flights.requests.get", return_value=mock_response) as mock_get:
        make_get_request("http://api.example.com/flights", params)
